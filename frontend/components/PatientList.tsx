...
export function PatientBrowse({ embedded = false }: { embedded?: boolean }) {
  ...
  return (
    <div
      className={`flex min-h-0 flex-1 flex-col lg:flex-row ${embedded ? "h-full" : "min-h-screen"}`}
    >
      <section className="flex w-full shrink-0 flex-col overflow-auto border-b border-zinc-200 lg:w-[24rem] lg:max-w-md lg:border-b-0 lg:border-r">
        <div className={embedded ? "space-y-3 p-4" : "space-y-3 p-4 md:p-5"}>
                ...
                <tbody className="divide-y divide-zinc-100">
                  {displayedPatients.map((p) => {
                    ...
                    return (
                      ...
                      >
                        <td className="px-3 py-2.5 font-medium text-zinc-900">{p.displayName}</td>
                        <td className="px-3 py-2.5 tabular-nums text-zinc-700">{p.dateOfBirth ?? "—"}</td>
                        <td className="hidden px-3 py-2.5 sm:table-cell">
                          <span
                            className={`inline-flex max-w-[10rem] items-center truncate rounded-full px-2 py-0.5 text-xs font-medium ring-1 ring-inset ${spiritualStatusBadgeClass(p.spiritualStatusKind)}`}
                          >
                            {p.heardGospelDate
      ...
      <main className="min-w-0 flex-1 overflow-auto bg-white">
        {!selected || draft == null ? (
          <div className="flex min-h-[20rem] flex-1 items-center justify-center p-8 lg:min-h-0">
            <p className="text-sm text-zinc-500">Select a patient to view their chart</p>
          </div>
        ) : (
