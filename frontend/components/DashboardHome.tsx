    ...
    <div className={embedded ? "space-y-6" : "mx-auto max-w-6xl space-y-8 px-4 py-8"}>
      {!embedded ? (
        <div className="flex flex-wrap items-end justify-between gap-4">
          <div>
            <h1 className="text-3xl font-semibold tracking-tight text-fc-ink">
              Mission dashboard
            </h1>
            <p className="mt-2 max-w-2xl text-sm leading-relaxed text-fc-ink-muted">
          ...
          <Link
            href="/patients"
            className="rounded-lg bg-fc-accent px-4 py-2 text-sm font-medium text-white shadow hover:bg-fc-accent-strong"
          >
            Patients
          </Link>
        </div>
      ...
      {loading && (
        <p className="text-sm text-fc-ink-subtle text-fc-ink-subtle">Loading metrics…</p>
      )}
      {error && (
      ...
      {data && sp && md && (
        <div className="grid gap-6 md:grid-cols-2">
          <section className="rounded-2xl border border-zinc-200 bg-white p-6 shadow-sm dark:border-zinc-800 dark:bg-zinc-950">
            <h2 className="text-sm font-semibold uppercase tracking-wide text-fc-ink-subtle text-fc-ink-subtle">
              Spiritual impact
            </h2>
            <p className="mt-1 text-xs text-fc-ink-subtle text-fc-ink-subtle">
            ...
            <dl className="mt-6 space-y-4">
              <div className="flex justify-between gap-4 border-b border-zinc-100 pb-3 dark:border-zinc-800">
                <dt className="text-sm text-fc-ink-muted text-fc-ink-subtle">Total patients</dt>
                <dd className="text-lg font-semibold tabular-nums text-fc-ink dark:text-zinc-100">
                  {sp.totalPatients.toLocaleString()}
                </dd>
              </div>
              <div className="flex justify-between gap-4">
                <dt className="text-sm text-fc-ink-muted text-fc-ink-subtle">Heard gospel</dt>
                <dd className="text-right">
                  <span className="text-lg font-semibold tabular-nums text-fc-accent dark:text-teal-400">
                    {sp.heardGospel.toLocaleString()}
                  </span>
                  <span className="ml-2 text-xs text-fc-ink-subtle">
                    ({pct(sp.heardGospel, sp.totalPatients)}%)
                  </span>
                </dd>
              </div>
              <div className="flex justify-between gap-4">
                <dt className="text-sm text-fc-ink-muted text-fc-ink-subtle">Hope / interest (not yet heard)</dt>
                <dd className="text-lg font-semibold tabular-nums text-fc-ink dark:text-zinc-100">
                  {sp.hopeWithoutHeard.toLocaleString()}
                </dd>
              </div>
              <div className="flex justify-between gap-4">
                <dt className="text-sm text-fc-ink-muted text-fc-ink-subtle">No spiritual record</dt>
                <dd className="text-lg font-semibold tabular-nums text-fc-ink dark:text-zinc-100">
          ...
          <section className="rounded-2xl border border-zinc-200 bg-white p-6 shadow-sm dark:border-zinc-800 dark:bg-zinc-950">
            ...
            <dl className="mt-6 space-y-4">
              <div className="flex justify-between gap-4 border-b border-zinc-100 pb-3 dark:border-zinc-800">
                <dt className="text-sm text-fc-ink-muted text-fc-ink-subtle">Patients with allergies documented</dt>
                <dd className="text-lg font-semibold tabular-nums text-fc-ink dark:text-zinc-100">
                  {md.patientsWithAllergiesDocumented.toLocaleString()}
                </dd>
              </div>
              <div className="flex justify-between gap-4">
                <dt className="text-sm text-fc-ink-muted text-fc-ink-subtle">With medical history</dt>
                <dd className="text-lg font-semibold tabular-nums text-fc-ink dark:text-zinc-100">
                  {md.patientsWithMedicalHistory.toLocaleString()}
                </dd>
              </div>
              <div className="flex justify-between gap-4">
                <dt className="text-sm text-fc-ink-muted text-fc-ink-subtle">With surgical history</dt>
                <dd className="text-lg font-semibold tabular-nums text-fc-ink dark:text-zinc-100">
                  {md.patientsWithSurgicalHistory.toLocaleString()}
                </dd>
              </div>
              <div className="flex justify-between gap-4">
                <dt className="text-sm text-fc-ink-muted text-fc-ink-subtle">Total visits (rows)</dt>
                <dd className="text-lg font-semibold tabular-nums text-fc-ink dark:text-zinc-100">
