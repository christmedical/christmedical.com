# public.prescriptions

## Labels

`clinical`

## Columns

| Name | Type | Default | Nullable | Children | Parents | Comment |
| ---- | ---- | ------- | -------- | -------- | ------- | ------- |
| client_updated_at | timestamp with time zone | CURRENT_TIMESTAMP | true |  |  |  |
| device_id | varchar(50) |  | true |  |  |  |
| directions | text |  | true |  |  |  |
| dose | varchar(255) |  | true |  |  |  |
| id | uuid | gen_random_uuid() | false |  |  |  |
| is_deleted | boolean | false | true |  |  |  |
| medication_name | varchar(255) |  | true |  |  |  |
| quantity | integer |  | true |  |  |  |
| server_restored_at | timestamp with time zone |  | true |  |  |  |
| status | varchar(50) |  | true |  |  |  |
| treatment_id | uuid |  | true |  | [public.treatments](public.treatments.md) |  |

## Constraints

| Name | Type | Definition |
| ---- | ---- | ---------- |
| prescriptions_pkey | PRIMARY KEY | PRIMARY KEY (id) |
| prescriptions_treatment_id_fkey | FOREIGN KEY | FOREIGN KEY (treatment_id) REFERENCES treatments(id) |

## Indexes

| Name | Definition |
| ---- | ---------- |
| prescriptions_pkey | CREATE UNIQUE INDEX prescriptions_pkey ON public.prescriptions USING btree (id) |

## Relations

```mermaid
erDiagram

"public.prescriptions" }o--o| "public.treatments" : "FOREIGN KEY (treatment_id) REFERENCES treatments(id)"
"public.procedure_details" }o--o| "public.treatments" : "FOREIGN KEY (treatment_id) REFERENCES treatments(id)"
"public.treatments" }o--o| "public.visits" : "FOREIGN KEY (visit_id) REFERENCES visits(id)"

"public.prescriptions" {
  timestamp_with_time_zone client_updated_at ""
  varchar_50_ device_id ""
  text directions ""
  varchar_255_ dose ""
  uuid id ""
  boolean is_deleted ""
  varchar_255_ medication_name ""
  integer quantity ""
  timestamp_with_time_zone server_restored_at ""
  varchar_50_ status ""
  uuid treatment_id FK ""
}
"public.treatments" {
  timestamp_with_time_zone client_updated_at ""
  varchar_50_ device_id ""
  text general_notes ""
  uuid id ""
  boolean is_deleted ""
  boolean is_flagged ""
  varchar_255_ provider_name ""
  timestamp_with_time_zone server_restored_at ""
  varchar_50_ type ""
  uuid visit_id FK ""
}
"public.procedure_details" {
  timestamp_with_time_zone client_updated_at ""
  varchar_50_ device_id ""
  uuid id ""
  boolean is_deleted ""
  jsonb metadata ""
  timestamp_with_time_zone server_restored_at ""
  uuid treatment_id FK ""
}
"public.visits" {
  text chief_complaint ""
  timestamp_with_time_zone client_updated_at ""
  varchar_50_ device_id ""
  text diagnosis_text ""
  uuid id ""
  boolean is_deleted ""
  varchar_255_ location_name ""
  uuid patient_id FK ""
  text referral_notes ""
  timestamp_with_time_zone server_restored_at ""
  smallint tenant_id ""
  uuid trip_id FK ""
  timestamp_with_time_zone visit_date ""
}
```

---

> Generated by [tbls](https://github.com/k1LoW/tbls)
